...
tryCatch({
  ...
  # Generate mock data if example data is not available
  set.seed(42)
  ...
  # Generate mock counts with correlated patterns
...
plot_taxa_network <- function(network, detect_communities = TRUE, node_size_by = "abundance") {
  ...
  p <- ggraph(network, layout = "fr") +
    geom_edge_link(aes(edge_width = weight, edge_colour = type, alpha = weight),
                  show.legend = TRUE) +
    scale_edge_colour_manual(values = c("positive" = "forestgreen", "negative" = "firebrick")) +
    scale_edge_width(range = c(0.1, 2)) +
    scale_edge_alpha(range = c(0.1, 1)) +
    geom_node_point(aes(size = size, fill = as.factor(community)), shape = 21, color = "white") +
    geom_node_text(aes(label = label), repel = TRUE, size = 3, alpha = 0.7) +
...
network_plot2 <- ggraph(taxa_network, layout = "circle") +
  geom_edge_link(aes(edge_width = weight, edge_colour = type),
                alpha = 0.7, show.legend = TRUE) +
  scale_edge_colour_manual(values = c("positive" = "forestgreen", "negative" = "firebrick")) +
  scale_edge_width(range = c(0.1, 2)) +
  geom_node_point(aes(size = size, fill = as.factor(community)), shape = 21, color = "white") +
  geom_node_text(aes(label = label), repel = TRUE, size = 3, check_overlap = TRUE) +
  scale_fill_brewer(palette = "Set1") +
  labs(
