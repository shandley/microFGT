...
# The mock data generator functions should now be available:
# - generate_mock_speciateit()
# - generate_mock_virgo()
# - generate_mock_valencia()
# - generate_mock_fgt_dataset()

# Fallback for older versions or development environment
if (!exists("generate_mock_speciateit")) {
  message("Loading mock generators from examples directory...")
  # Try loading from the example files
  mock_file <- system.file("examples", "mock_data", "mock_generators.R", package = "microFGT")
  if (file.exists(mock_file)) {
    source(mock_file)
  } else if (file.exists("R/mock_data.R")) {
    # Development environment
    source("R/mock_data.R")
  } else {
    stop("Could not find mock data generators. Please ensure you have the latest version of microFGT.")
  }
}
